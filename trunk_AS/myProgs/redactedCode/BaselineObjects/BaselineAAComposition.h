...
namespace MSL { 
	class BaselineAAComposition: public OneBodyInteraction {
		...
		public:
			...
			std::vector<std::vector<std::string>> getPenaltyAAs();
			std::vector<std::vector<double>> getPenaltyFractions();
			std::map<std::string,double> getMap();
			std::string toString();
			void printParameters();
			void editProperty(std::string, std::vector<std::string> AAs);
			void defaultPropertyMap();
			std::map<std::string,vector<string>> getPropertyMap();
			std::vector<bool> getPropertyBooleans();
			...
			//TODO: add the below into the cpp file
					
		private:
			void setup(System *sys, string type, double factor);
			void copy(BaselineAAComposition & _interaction);
			static const std::string typeName;
			std::vector<std::vector<std::string>> penAAs;
			std::map<std::string,double> pMap;
			std::vector<std::string> polarVec {"CYS", "GLU", "ASP", "SER", "TYR", "MET"};
			std::vector<std::string> nonpolarVec {"ALA", "PHE", "ILE", "LEU", "TRP", "VAL", "GLY"};
			std::vector<std::string> chargeVec {"GLU", "ASP", "LYS", "ARG", "HIS"};
			std::vector<std::string> aroVec {"PHE", "HIS", "TYR", "TRP"};
			std::map<std::string,vector<string>> propertyVecMap;
			std::string penaltyFile;
			std::string type;
			std::vector<bool> hasProperty; //defaults to false, expecting a type of AA, not multiple (like the original I wrote)
			std::vector<std::vector<double>> penFracs;
			std::string function;
			System *sys;
			double energy;
			double factor;
	...
	inline std::vector<bool> BaselineAAComposition::getPropertyBooleans(){ return hasProperty;}
	inline void BaselineAAComposition::setFactor(int _factor){ factor = _factor;}
	inline std::vector<std::vector<std::string>> BaselineAAComposition::getPenaltyAAs(){ return penAAs;}
	inline std::vector<std::vector<double>> BaselineAAComposition::getPenaltyFractions(){ return penFracs;}
	...
	inline std::map<std::string,vector<string>> BaselineAAComposition::getPropertyMap(){ return propertyVecMap;} 
	
	inline void BaselineAAComposition::defaultPropertyMap(){ 
		propertyVecMap.clear();
		propertyVecMap["POLAR"] = polarVec;
		propertyVecMap["NONPOLAR"] = nonpolarVec;
		propertyVecMap["CHARGE"] = chargeVec;
		propertyVecMap["AROMATIC"] = aroVec;
	}
}
