...
#ifndef BASELINEOUTERPAIRINTERACTION_H
#define BASELINEOUTERPAIRINTERACTION_H
...
namespace MSL { 
	class BaselineOuterPairInteraction: public TwoBodyInteraction {
		...
		public:
			BaselineOuterPairInteraction();
			BaselineOuterPairInteraction(Atom & _d1, Atom & _d2, double _energy);

			// should implement an operator= as well 
			BaselineOuterPairInteraction(const BaselineOuterPairInteraction & _interaction);
			~BaselineOuterPairInteraction();
			...
			friend std::ostream & operator<<(std::ostream &_os, BaselineOuterPairInteraction & _term) {_os << _term.toString(); return _os;};
			std::pair<double,std::vector<double> > partialDerivative();

					
		private:
			void setup(Atom * _d1, Atom * _d2, double _energy);
			void copy(const BaselineOuterPairInteraction & _interaction);
			static const std::string typeName;
	};

	inline std::string BaselineOuterPairInteraction::toString() { 
	...
	inline std::string BaselineOuterPairInteraction::getName() const {return typeName;}
	inline std::pair<double,std::vector<double> > BaselineOuterPairInteraction::partialDerivative() {
