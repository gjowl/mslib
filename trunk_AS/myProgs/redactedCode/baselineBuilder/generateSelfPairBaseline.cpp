...
string programName = "generateSelfPairBaselinevdW";
...
string programDate = "27 August 2021";
...
void printEnerFile(string _seq, Options _opt, vector<double> &_ener, vector<double> &_hEner, vector<double> &_vdWEner, int _seqNumber, bool _selfEner, ofstream &_out){
	if (_selfEner == true){
		...
		if (_seqNumber == 0 && _opt.seed == 0){
			_out << "Pair: Distance: Position1: Position2: Energy: Hbond: vdW" << endl;//could be interesting to add rotamer number to this
		}
		vector<string> pairs;
		vector<double> dists;
		for (uint i=0; i<_seq.length(); i++){
		...
		for (uint i=0; i<pairs.size(); i++){
			...
			_out << pairs[i] << sp << dists[i] << sp << pos1 << sp << pos2 << sp << _ener[i] << sp << _hEner[i] << sp << _vdWEner[i] << endl;
...
int main(int argc, char *argv[]){
	...
	**********************************************************************************/
	...
	while (seqAccept < seqNumber){
		...
		cout << seq << endl;
		string polySeq = convertToPolymerSequenceNeutralPatch(seq, opt.thread);//To have neutral patches at the ends of my sequences to prevent any hydrogen bonding that could affect energy score
		...
		//add in cutoffs for nonbonded energy terms TODO: add this portion and updatedNonBonded to any other code I use to measure energies
		...
		vector<double> AAHbond = calcBaselineEnergies(sys, seq.length(), totalProt, opt.thread);
		vector<double> AAPosHbond = calcBaselinePositionEnergies(sys, seq.length(), totalProt, opt.thread);
		vector<double> pairAAHbond = calcPairBaselineEnergies(sys, opt, seq.length(), totalProt, opt.thread);
		
		/******************************************************************************
		 *                 === CALCULATE ENERGIES FOR ERASED TERM ===
		 ******************************************************************************/
		vector<double> AAvdW     = getSubtractBetweenVectors(AAener, AAHbond);
		vector<double> AAPosvdW  = getSubtractBetweenVectors(AAPosEner, AAPosHbond);
		vector<double> pairAAvdW = getSubtractBetweenVectors(pairAAEner, pairAAHbond);
		...
		printEnerFile(seq, opt, AAener, AAHbond, AAvdW, seqAccept, true, sout);
		printSeqFile(PS, seq, pairAAEner, pairAAHbond, pairAAvdW, totalEnergy, hbond, vdw, seqAccept, false, spout);
		printEnerFile(seq, opt, pairAAEner, pairAAHbond, pairAAvdW, seqAccept, false, pout);
		seqAccept++;
		sys.reset();
	}
	time (&endTime);
...
Options parseOptions(int _argc, char * _argv[], Options defaults){
	...
	if (OP.fail()) {
		opt.warningFlag = true;
		opt.warningMessages += "thread not specified, default to 25\n";
		opt.thread = 25;
	}
