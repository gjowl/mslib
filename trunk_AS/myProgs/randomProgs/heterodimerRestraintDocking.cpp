...
string programName = "mutationRestraintDocking";
...
string programAuthor = "Samantha Anderson; Gilbert Loiseau edited this to make a hetero version for this docking algorithm";
string programVersion = "1.0.3";
string programDate = "04 March 2020";
...
struct Options {
	...
	//Bool to use CATM backbone with slightly adjusted coordinates
	bool useCatmBackbone;
...
int main (int argc, char *argv[]) {
	...
	string logFile = opt.outputDir + "/" + to_string(opt.replicateStartNum) + ".log";
	ofstream lout;
	if (opt.verbose){
	...
	// Make List file
	string listFile = opt.outputDir + "/energies.list";
	ofstream eOut;
...
Options parseOptions(int _argc, char * _argv[], Options defaults) {
	...
	opt.required.push_back("startT");
	opt.required.push_back("endT");
	opt.required.push_back("mcCycles");
	...
	opt.allowed.push_back("useCatmBackbone");
...
//	}
	...
	if (OP.fail()) {
		opt.errorMessages += "start temp not specified!\n";
		opt.errorFlag = true;
	}

	opt.endT = OP.getDouble("endT");
	if (OP.fail()) {
		opt.errorMessages += "end temp not specified!\n";
		opt.errorFlag = true;
	}                                
...
//	}
	...
	if (OP.fail()) {
		opt.warningMessages += "mcShape not specified, default to EXPONENTIAL\n";
		opt.warningFlag = true;
		opt.mcShape = 2;
	}
	...
	if (OP.fail()) {
		opt.warningMessages += "Max steps without improvement not specified! Default to 300\n";
		opt.warningFlag = true;
		opt.convergedSteps = 300;
	}
	...
	opt.useCatmBackbone = OP.getBool("useCatmBackbone");
	if (OP.fail()) {
		opt.useCatmBackbone = false;
		opt.warningMessages += "useCatmBackbone not specified! Default to false ";
		opt.warningMessages += "\n";
		opt.warningFlag = true;
	}
