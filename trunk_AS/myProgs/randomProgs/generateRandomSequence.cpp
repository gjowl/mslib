...
string programDescription = "This program designs randomized sequences to calculate baselines for future sequence design; start for the 31st that still seg faults";
string programAuthor = "Gilbert Loiseau";
string programVersion = "1";
string programDate = "31 July 2019";
...
int main(int argc, char *argv[]){
	...
	**********************************************************************************/
	...
	//for (uint a=0; a<opt.seqNumber; a++){
		...
		vector<string> weightedAAs = weightAAs(opt.ids, opt.weights, RNG);
		...
		if (opt.backboneLength > weightedAAs.size()){
			...
			if (sys.calcEnergy() < 0 && Eset->getTermEnergy("CHARMM_VDW") < 0){
				 ...
				 ******************************************************************************/
				double totalProt = 0;
				vector<double> AAener =	calcBaselineEnergies(sys, opt.backboneLength, totalProt);
				...
				if (a == 0){
					...
					nout << PS;
					nout << "AA      Position      Energy" << endl;//could be interesting to add rotamer number to this
					rout << "AA      Position      Energy" << endl;//could be interesting to add rotamer number to this
					for (uint i=0; i<opt.backboneLength; i++){
						nout << seq[i] << ":     " << i+1 << ":            " << AAener[i] << endl;
						rout << seq[i] << ":     " << i+1 << ":            " << AAener[i] << endl;
					...
					nout << "H-bond Energy: " << Eset->getTermEnergy("SCWRL4_HBOND") << endl;
					nout << "VDW Energy: " << Eset->getTermEnergy("CHARMM_VDW") << endl << endl;
				}
				else{
					nout << "Sequence: " << a+1 << endl;
					nout << PS;
				...
				if (Eset->getTermEnergy("CHARMM_VDW") > 0){
					cout << "VDW Energy " << Eset->getTermEnergy("CHARMM_VDW") << " is too high; go to next random sequence." << endl;
				}
