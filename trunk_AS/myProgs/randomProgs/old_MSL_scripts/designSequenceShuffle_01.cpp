...
int main(int argc, char *argv[]){
	...
	System pdb;
	pdb.readPdb("/exports/home/gloiseau/Generated_PDBs/sequenceDesign/11_02_2019/SCMFstate_1.pdb", true);
	
	cout << pdb.chainSize() << endl;
	Chain &chain1 = pdb.getChain(0);

	string seq1 = convertPolymerSeqToOneLetterSeq(chain1);
	cout << seq1 << endl;

	standardizeResidueList(chain1, opt.ids);

	string poly2 = generateMultiIDPolymerSequence(seq1, 28, opt.ids);//this thread number must be the same as the one from the vdwSequenceDesign, otherwise it won't transform properly
	cout << poly2 << endl;
	...
	Eset->saveEnergySubset("CHARMM_VDW");
	fout << "Total interactions calculated: " << Eset->getTotalNumberOfInteractionsCalculated() << endl;
	...
	//
	//sys.setLinkedPositions(pos2String);//having this uncommented should make it go faster (homodimer)

	string origSeq = convertPolymerSeqToOneLetterSeq(chainA);
	string seq = origSeq;
	
	cout << sys.calcEnergy() << endl;
	cout << Eset->getSummary() << endl;
	//despite having the same structure (I think? it may not be optimized?) the energies are very different
	/******************************************************************************
