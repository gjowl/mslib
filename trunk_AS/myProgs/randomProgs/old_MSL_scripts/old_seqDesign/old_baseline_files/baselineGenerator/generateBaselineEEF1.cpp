...
vector<double> calcBaselineEnergies(System &_sys, int _seqLength, double &_totalProt, double &_interactions, double &_refInteractions){
	...
	for (uint i=0; i<_seqLength; i++){
		...
		double resi = _sys.calcEnergy("resi");
		cout << eset->getSummary() << endl;
		cout << eset->getTermEnergy("CHARMM_EEF11") << endl;
		imm1ener = imm1ener + eset->getTermEnergy("CHARMM_EEF11");
		inter = inter + eset->getTermNumberOfInteractionsCalculated("CHARMM_EEF1");
		cout << resi << endl;
	...
	for (uint i=0; i<_seqLength; i++){
		...
		for (uint j=i+1; j<_seqLength; j++){
			...
			//if (dist >= 0){
				...
				imm1ener = imm1ener + eset->getTermEnergy("CHARMM_EEF1");
				inter = inter + eset->getTermNumberOfInteractionsCalculated("CHARMM_EEF1");
				//cout << eset->getSummary() << endl;
...
int main(int argc, char *argv[]){
	...
	**********************************************************************************/
	...
	string soutName  = dir + "/EEF1_Self_Energies_" + opt.runNumber + ".out";
	string poutName  = dir + "/EEF1_Pair_Energies_" + opt.runNumber + ".out";
	...
	while (seqAccept < seqNumber){
		...
		CSB.setBuildTerm("CHARMM_EEF1", true);
		...
		Eset->setTermActive("CHARMM_EEF1REF", true);
		Eset->setTermActive("CHARMM_EEF1", true);
		
		// Set weights
		//Eset->setWeight("CHARMM_VDW", opt.weight_vdw);
		//Eset->setWeight("SCWRL4_HBOND", opt.weight_hbond);
		Eset->setWeight("CHARMM_EEF1", 1);
		Eset->setWeight("CHARMM_EEF1REF", 1);
		...
		cout << Eset->getSummary() << endl;
		double EEF1 = Eset->getTermEnergy("CHARMM_EEF1") ;
		...
		double totalEEF1Energy  = baselineEner + baselinePairEner;
	
		double numInteractions = interactions + refInteractions;
		cout << "Total Energy: " << totalEnergy << endl;
		cout << "BaselineEEF1 Energy: " << baselineEner << endl;
		cout << "BaselinePairEEF1 Energy: " << baselinePairEner << endl;
		cout << "Baseline Addition: " << totalEEF1Energy << endl;
		cout << "BaselineEEF1 Energy: " << totalSelfPair << endl;
		cout << "BaselinePairEEF1 Energy: " << totalPair << endl;
		cout << "Ref Interactions: " << refInteractions << endl;
		cout << "EEF1 Interactions: " << interactions << endl;
