...
vector<double> calcBaselineEnergies(System &_sys, int _seqLength, double &_totalProt, double &_interactions, double &_refInteractions){
	...
	for (uint i=0; i<_seqLength; i++){
		...
		double resi = _sys.calcEnergy("resi")/2;//Was reading the implementation of Franklin2019 and realized they said that pairwise interactions are difficult...so what if I ONLY use a self term? Will it be more accurate? I think it was a self term included in the runs I did over summer? I can check but pretty sure
		//cout << eset->getSummary() << endl;
		//cout << eset->getTermEnergy("CHARMM_IMM1REF") << endl;
...
int main(int argc, char *argv[]){
	...
	**********************************************************************************/
	...
	while (seqAccept < seqNumber){
		...
		double totalIMM1Energy  = baselineEner;
		...
		cout << "IMM1 Interactions: " << interactions << endl;
		...
		//printSeqFile(PS, seq, pairAAener, totalEnergy, seqAccept, false, spout);
