...
string programName = "baselineSelfPairComparison_01";
string programDescription = "This program is an edited, more organized version of baselineComparison that designs randomized sequences to evaluate baseline energies from generateBaselineSelfPair.cpp against monomer sequences";
string programAuthor = "Gilbert Loiseau";
string programVersion = "3";
string programDate = "8 September 2020";
...
double seqPairEnergy(string &_seq, map<string, vector<double>> _pairEner){
	double ener = 0;
...
vector<double> printSelfBaselineEnergy(string &_seq, map<string, double> _selfEner){
	vector<double> ener;
	for (uint i=0; i<_seq.length(); i++){
		string aa(1, _seq[i]);
		for (map<string, double>::const_iterator it = _selfEner.begin(); it != _selfEner.end(); ++it){
...
vector<double> printPairBaselineEnergy(string &_seq, map<string, vector<double>> _pairEner){
	vector<double> ener;
	vector<string> pairs1;
	vector<string> pairs2;
	vector<int> dists;
...
map<string, vector<double>> makeBaselineMapPos(string _file){
...
void printEnerFile(string _seq, vector<double> &_ener, vector<double> &_bEner, int _seqNumber, bool _selfEner, ofstream &_out){
	if (_selfEner ==true){
		if (_seqNumber == 0){
		...
		if (_seqNumber == 0){
			_out << "Pair:     Distance:    Energy:      Baseline" << endl;
		}
		vector<string> pairs;
		vector<double> dists;
		for (uint i=0; i<_seq.length(); i++){
		...
		for (uint i=0; i<pairs.size(); i++){
			_out << pairs[i] << spacer << dists[i] << spacer << _ener[i] << spacer << _bEner[i] << endl;
			//_out << pairs[i] << ": " << _bEner[i] << endl;
		}
	}
}
...
int main(int argc, char *argv[]){
	...
	**********************************************************************************/
	...
	string eoutName   = dir + "/energyComparison_" + opt.runNumber + ".out";
	 ...
	 ******************************************************************************/
	map<string, double> selfNoPos       = makeBaselineMap("/exports/home/gloiseau/mslib/trunk_AS/myProgs/gloiseau/2020_09_24_meanSelf.txt");
	map<string, vector<double>> selfPos = makeBaselineMapPos("/exports/home/gloiseau/mslib/trunk_AS/myProgs/gloiseau/mean_selfPosition.txt");
	map<string, vector<double>> pair1   = makeBaselineMapPos("/exports/home/gloiseau/mslib/trunk_AS/myProgs/gloiseau/2020_09_24_meanPair.txt");
	
	//for (map<string, vector<double>>::const_iterator it = pair1.begin(); it != pair1.end(); ++it){
	//	cout << it->first << endl;
	//}
	...
	while (seqAccept < seqNumber){
		 ...
		 ******************************************************************************/
		double seqSelfEner = 0;
		seqSelfEner = seqSelfEnergy(seq, selfNoPos);
		vector<double> selfBaselines = printSelfBaselineEnergy(seq, selfNoPos);
		double seqSelfPosEner = 0;
		seqSelfPosEner = seqSelfPositionEnergy(seq, selfPos);
		double seqPairEner = 0;
		seqPairEner = seqPairEnergy(seq, pair1);
		vector<double> pairBaselines = printPairBaselineEnergy(seq, pair1);
		...
		//TODO: add in a check for atoms not being built from generateSelfPairBaselinevdW_01.cpp
		 ...
		 ******************************************************************************/
		vector<double> selfEner = calcBaselineEnergies(sys, seq.length()); //reapplying this function from generateSelfPairBaselinevdW_01.cpp to extract self and pair energies for e`ach sequence	
		vector<double> pairEner = calcPairBaselineEnergies(sys, seq.length()); //reapplying this function from generateSelfPairBaselinevdW_01.cpp to extract self and pair energies for e`ach sequence	
		...
		for (uint i=0; i != seq.length(); i++){
			for (map<char,double>::iterator it = idEner.begin(); it != idEner.end(); it++) {
				if (it->first == seq[i]){
					//bsout << i+1 << ": " << it->first << " " << it->second << endl;
		...
		//bsout << "Sequence " << a+1 << ": " << seq << endl;
		//bsout << "Total Energy: " << totalEnergy << endl;
		//bsout << "Baseline Monomer Energy: " << monomerEner << endl;
		//bsout << "Total - Baseline: " << totalEnergy-monomerEner << endl;
		cout << "Total Energy: " << totalEnergy << endl;
		cout << "Baseline Monomer Energy: " << monomerEner << endl;
		cout << "Total - Baseline: " << totalEnergy-monomerEner << endl;
		cout << "Self Energy: " << seqSelfEner << endl;
		cout << "Pair Energy: " << seqPairEner << endl;
		cout << "Total - Self Position Baseline: " << totalEnergy-seqSelfPosEner << endl;
		cout << "SelfPair Energy: " << seqBaselineEnergy << endl;
		cout << "Total - SelfPair Energy: " << totalEnergy-seqBaselineEnergy << endl;
		
		double baselineDif = totalEnergy - monomerEner;
		double selfPosDif = totalEnergy - seqSelfPosEner;
		 ...
		 ******************************************************************************/
		if (seqAccept == 0){
			eout << "Baseline:     SelfPos:    SelfPair:" << endl;
			bsout << "Monomer:     Baseline:     SelfPos:    SelfPair:" << endl;
		}
		eout << baselineDif << ":     " << selfPosDif << ":    " << selfPairDif << endl;
		bsout << totalEnergy << ":    " << monomerEner << ":     " << seqSelfPosEner << ":    " << seqBaselineEnergy << endl;
		if (pairEner.size() > pairBaselines.size()){
			while (pairEner.size() != pairBaselines.size()){
			...
			if (pairEner.size() < pairBaselines.size()){
				cout << "Pair size is not equal to baseline size: Pair " << pairEner.size() << "vs Baseline " << pairBaselines.size() << "." << endl;
				break;
			}
		}
		printEnerFile(seq, selfEner, selfBaselines, seqAccept, true, sout);
		printEnerFile(seq, pairEner, pairBaselines, seqAccept, false, pout);
		seqAccept++;
		sys.reset();
	}
	time (&endTime);
