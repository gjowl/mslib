		...
		if (tokens[0] == "END_PENALTY_DEFINITION"){
			...
			for (uint j=1; j<tokens.size(); j++){
				if (tokens[0] == "TYPE"){
					...
					penAAs.push_back(MslTools::toUpper(tokens[j]));
				}
				if (tokens[0] == "PENALTIES"){
					ener.push_back(MslTools::toDouble(tokens[j]));
				}
				if (tokens[0] == "FRACTION"){
					penFracs.push_back(MslTools::toDouble(tokens[j]));
				}
			}
		}
	}
...
double BaselineAAComposition::calcMultiplicationFactor(double _count, double _factor, string type){
...
void BaselineAAComposition::buildInteraction(){
	...
	energy = 0;
	setCurrentSeq();
	//cout << penAAs.size() << endl;

	for (uint i=0; i<penAAs.size(); i++){
		double count = 0;
		double f = factor;
		for (uint j=0; j<pSeq.size(); j++){
			if (penAAs[i] == pSeq[j]){
				count++;
				//cout << penAAs[i] << ": " << pSeq[j] << endl;
				//cout << count << endl;
				//cout << i << ": " << j << endl;
			}
		}
		if (count > penFracs[i]){
			f = calcMultiplicationFactor(count-penFracs[i], f, type);
			energy += pMap.at(penAAs[i])*f;
		}
	}
	if (params.size() == 0){
		params.push_back(energy);
