...
string programName = "baselineSelfPairComparison";
string programDescription = "Final version of baselineSelfPairComparison: This program generates random monomer sequences and tests the baseline energies from generateSelfPairBaseline, aiming to see if there is a good correlation between the calculated energy scores and the scores summed from the baseline energies. This code is test code for the baselines prior to using them for seqDesign. If there is good correlation, these energies can be used to estimate the monomer energy during design, allowing the monomer to be taken into account without separate calculation during the design process.";
					...
					if (d <= 10){
						for (uint k=0; k < (*p2)->identitySize(); k++){
							...
							try{
								...
								cout << baseId1 << ": " << baseId2 << " = " << ener << endl;
...
int main(int argc, char *argv[]){
	...
	**********************************************************************************/
	...
	while (seqAccept < seqNumber){
		...
		map<string, double> selfMap = readSingleParameters("/exports/home/gloiseau/mslib/trunk_AS/DesignFiles/2021_08_30_selfBaselineLILI.txt");
		map<string,map<string,map<uint,double>>> pairMap = readPairParameters("/exports/home/gloiseau/mslib/trunk_AS/DesignFiles/2021_08_30_pairBaselineLILI.txt");
