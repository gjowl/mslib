...
string programDescription = "This program examines the interhelical giometry of pairs of interaction helices in structures\
							edited version for JC to only output the geometry of the interacting helices, commented out\
							unnecessary outputs";
...
//	printICOptions(opt, fout);
	...
	// split pdbname from extension
	string pdbName = MslTools::getFileName(argv[1]);
	...
	// concatenate pdbName to the report name; only this since this is the important output for JC
	string output = pdbName + "_pairGeometryReport";
	writeReport(output, opt.outputdir, ss);
