...
string programName = "pdbBBOptimize_interhelicalHbond";
string programDescription = "Reads a PDB, does a backbone repack, and calculates the energy of the interhelical interactions.\
 Can also take an input sequence using input pdb as a backbone. Edited version of pdbBBRepack";
...
vector<double> calcDimerInterhelicalEnergies(System &_sys, string _sequence, ofstream &_out);
...
void backboneOptimizer(Options &_opt, RandomNumberGenerator &_RNG, string _sequence, PolymerSequence _PS, AtomPointerVector &_startPdb, map<string,double> _startGeometry,
 map<string, map<string, double>> &_sequenceEnergyMapFinal, ofstream &_eout){
	...
	writePdb(sys, _opt.outputDir, "test1");
	sys.assignCoordinates(_startPdb,false);
	sys.buildAllAtoms();
	writePdb(sys, _opt.outputDir, "test2");
    ...
    cout << "Dimer Energy: " << dimer << endl;
    sys.printEnergySummary();
	writePdb(sys, _opt.outputDir, "test4");
	/******************************************************************************
...
void backboneOptimizeMonteCarlo(Options &_opt, System &_sys, SelfPairManager &_spm, map<string,double> &_energyMap,
 ...
 string _outputDir, int _rep, ofstream &_sout){
	...
	PDBWriter writer;
	writer.open(_opt.outputDir + "/bbOptimizationTrajectory_"+to_string(_rep)+".pdb");
	// loop through the MC cycles for backbone repacks
	bbout << "Starting Repack Cycles" << endl; 
	ofstream bbout_interhelical;
	string bboutFile = _opt.outputDir + "/bbOptimization_"+to_string(_rep)+".config";
	bbout_interhelical.open(bboutFile.c_str());
	while(!MCMngr.getComplete()) {
		...
		if (!MCMngr.accept(currentEnergy)) {
			...
			// TODO: if accept, get the enrgy of each of the interhelical interactions and add here; also before make sure to optimize not by total energy,
			// but by interhelical energy; use temperature 500
			calcDimerInterhelicalEnergies(_sys, _opt.sequence, bbout_interhelical);
...
vector<double> calcMonomerIndividualEnergies(System &_sys, int _seqLength){
	vector <double> ener;
...
vector<double> calcDimerInterhelicalEnergies(System &_sys, string _sequence, ofstream &_out){
	...
	int seqLength = _sequence.length();
	for (uint i=0; i<seqLength; i++){
		string resi1A = "resi1A, chain A and resi ";
		string resi1B = "resi1B, chain A and resi ";
		string num1 = to_string(i+1);
		sel.select(resi1A += num1);
		sel.select(resi1B += num1);
		for (uint j=i+1; j<seqLength; j++){
			int dist = j-i;
			if (dist <= 10){
				string resi1A = "resi2A, chain A and resi ";
				string resi1B = "resi2B, chain B and resi ";
				string num2 = to_string(j+1);
				sel.select(resi1A += num2);
				sel.select(resi1B += num2);
				double pair1 = _sys.calcEnergy("resi1A", "resi2A");
				double pair2 = _sys.calcEnergy("resi1A", "resi2B");
				double pair3 = _sys.calcEnergy("resi1B", "resi2A");
				double pair4 = _sys.calcEnergy("resi1B", "resi2B");
				ener.push_back(pair1);
				ener.push_back(pair2);
				ener.push_back(pair3);
				ener.push_back(pair4);
			}
			else{
				j = seqLength;
			}
		}
		// TODO: save anything that has an interhelical hbond or vdw interaction below; maybe save as a vector and then output to a file?
	}
	sel.clearStoredSelections();
	return ener;
}
