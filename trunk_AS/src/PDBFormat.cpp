...
PDBFormat::AtomData PDBFormat::parseAtomLine(const string &_pdbAtomLine){
	...
	try {
		...
		if (lineLength >= E_OCCUP && MslTools::trim(_pdbAtomLine.substr(S_OCCUP, L_OCCUP)) != "")     atom.D_OCCUP          = MslTools::toDouble(MslTools::trim(_pdbAtomLine.substr(S_OCCUP, L_OCCUP)), "Occupation problem");
