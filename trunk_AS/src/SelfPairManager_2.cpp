...
void SelfPairManager::calculateFixedEnergies() {
	...
	for (map<string, vector<Interaction*> >::iterator k=subdividedInteractions[0][0][0][0].begin(); k!= subdividedInteractions[0][0][0][0].end(); k++) {
		...
		for (vector<Interaction*>::iterator l=k->second.begin(); l!= k->second.end(); l++) {
			...
			//thread{threadGetEnergy, &(*l); //this one works
			//threads.push_back(thread(threadGetEnergy, &E));
			//threads.push_back(thread(threadGetEnergy, &l, ref(E)));
			//E += (*l)->getEnergy();
		}
		for(auto &t : threads) {
...
double SelfPairManager::computePairE(unsigned pos1, unsigned rot1, unsigned pos2, unsigned rot2, string _term) {
	...
	if(!onTheFly) {//Added in to fix an onTheFly issue in the code 2021-10-12
		if (_term == "") {
...
vector<vector<unsigned int>> SelfPairManager::getBestSCMFBiasedMCStates() {
	return bestSCMFBiasedMCstates;
}
...
void SelfPairManager::runOptimizer() {
	...
	if(onTheFly && (runDEE || runSCMF)) {//on 1-14-21 I flipped this and the following for loop (calculateEnergies needs to set the selfE so that getAliveCombinations() isn't just 1
		onTheFly = false;
	...
	//cout << "Final: " << finalCombinations << endl;
	...
	cout << "Final Combinations: " << finalCombinations << endl; 
}
...
void SelfPairManager::runSelfConsistentMeanField() {
	...
	if(runSCMFBiasedMC) {
		...
		//bestSCMFBiasedMCstates  = SCMF.runMC1(mcStartT, mcEndT, mcCycles, mcShape, mcMaxReject, mcDeltaSteps, mcMinDeltaE);
		if(verbose) {
	...
	while(currCycle < _cycles){
		...
		if(verbose) {
			...
			for(int i = 0; i < state.size(); i++) {
				//TODO: document changing these lines to prevent "," at end of cycle
				if (i != state.size()-1){
					cout << state[i] << "," ;
				}
				else{
