...
bool BaselineEnergyBuilder::buildInteractions() {
	...
	for(vector<Position*>::iterator p = positions.begin(); p != positions.end(); p++) {
		for(unsigned int i = 0; i < (*p)->identitySize(); i++) {
			...
			//TODO: 2020_10_16 need to edit  to take into account different energies for the Leu at termini
			...
			//	if (p < positions.begin()+3 || p > positions.end()-3){//This doesn't work, replaces the CA energy for all Leu and makes energy too high...is there a way to add to baseline energy in Eset?
			//		a = &res.getAtom("CA");
			//		ESet->addInteraction(new BaselineInteraction(*a,1.2329));
			//		it == pBaselineEnergies[baseId].end();
			//	}
			//	else{
