...
class SelfConsistentMeanField {
	public:
		...
		std::vector<vector<unsigned int>> runMC1(double _startingTemperature, double _endingTemperature, int _scheduleCycles, int _scheduleShape, int _maxRejectionsNumber, int _convergedSteps, double _convergedE); // run an SCMF Biased Monte Carlo search and returns all states with the best Energy
