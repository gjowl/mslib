...
int main(int argc, char *argv[]) {	
		...
		for (uint c = 0; c < sys.chainSize();c++){
		  ...
		  if (opt.regex == ""){
		    for (uint a = 0; a < ch.atomSize();a++){
		      ...
		      tmp->removeAllAltConformations(); //SGFC seg faults when loading checkpts with alt confs...
	...
	PDBWriter writer("/tmp/fragdb.pdb");
	if(writer.write(results)) {
		cout << "Written successfully!\n";
	} else {
		cout << "ERROR: PDB NOT WRITTEN!\n";
	}

	// Write out binary checkpoint file.
	cout << "Size of archive: " << results.size() << " atoms" << endl;
