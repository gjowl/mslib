...
int main(int argc, char *argv[]) {
	...
	cout << "Loading rotamers complete!" << endl;
	cout << "Total number of rotamers on each position: " << endl;
	for (unsigned int i = 0; i < sys.positionSize(); i++) {
		Position &pos = sys.getPosition(i);
		cout << pos.getPositionId() << ", " << pos.getResidueName() << ": " << pos.getTotalNumberOfRotamers() << endl;
