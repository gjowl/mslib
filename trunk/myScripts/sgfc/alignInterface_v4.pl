...
open LOGFILE, ">./align_logfile.txt";
my $mainDir = shift;
my $useAll;
my $outputFile = shift;
if (!$outputFile) {
...
sub max_Align () {
	...
	my ($cross1, $cross2, $intLeft1, $intLeft2, $intRight1, $intRight2);
	if ($interface1 =~ /22..22/) {
		$cross1 = pos ($interface1);
		$intLeft1 = $cross1 - 9;
		if ($intLeft1 < 0) {
			$intLeft1 = 0;
		}
		$intRight1 = pos ($interface1) + 4;
		if ($intRight1 > length ($interface1)) {
			$intRight1 = length ($interface1);
		}
	}
	if ($interface2 =~ /22.22/) {
