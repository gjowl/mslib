...
int main(int argc, char *argv[]) {
	...
	duplicateHelix(sys, CSB, opt.numHelices-1);
	CSB.updateNonBonded();
...
//	cout << sys << endl;
	...
	/******************************************************************************
	 *                     === CALCULATE INTERACTIONS AND STARTING POSITIONS ===
	 ******************************************************************************/
...
//
				...
				counter++;
				if (counter == 1 || counter%1029==0) {
					string name = opt.outputDir + "/" + printInfo(axialRotate,crossingAngle,relZ,shiftedAxialRotate,zShift,xShifts,energies,hBonds) + ".pdb";
					sys.applySavedCoor("initialState");
					transformCoiledCoil(sys,zShift,crossingAngle,axialRotate,bestx,trans,origin,zAxis,xAxis);
					sys.writePdb(name);
				} else {
...
void transformCoiledCoil(System &_sys, double _zShift,double _crossingAngle, double _axialRotate,double _xShift,Transforms& _trans,CartesianPoint& _origin, CartesianPoint& _zAxis, CartesianPoint& _xAxis) {
	...
	applyRotationalSymmetry(_sys, _trans);
...
string printInfo(double axialRotate,double crossingAngle,double zShift,double relAxRot,double relZShift,vector<double> xShifts,vector<double> energies,vector<map<string,double> > hBonds) {
	...
	char info[100];
	sprintf(info,"%03.3f\t" "%03.1f\t" "%03.3f\t" "%03.3f\t" "%03.3f\t" "%03.1f", axialRotate, crossingAngle, zShift, relAxRot, relZShift, xShifts[minIdx]);
	cout << info << "\t" << hBonds[minIdx].size() ;
	...
	cout << endl;
	char name[100];
	sprintf(name,"model_%03.3f_" "%03.1f_" "%03.3f_" "%03.3f_" "%03.3f_" "%03.1f", axialRotate, crossingAngle, zShift, relAxRot, relZShift, xShifts[minIdx]);
	return string(name);
...
bool duplicateHelix (System &_sys, CharmmSystemBuilder &_csb, int _helices=1) {
	for (unsigned int i = 0; i < _helices; i++) {
