...
struct Options{
	...
	string logFile; 	
	string outputFile;	//writes output to a text file
	string pdbDir;		//place to write pdb files for solutions
...
*/
		...
		if (accept == true) {
			...
			sys.setActiveRotamers(SPM.getMinStates()[0]);
			if(opt.pdbDir != "") {
				sys.writePdb(opt.pdbDir + "evolutionAccept_" + MslTools::intToString(mutations) + ".pdb");
			}
...
//Builds a residue mask so that each position only has one specified residue at that position.	
std::vector< std::vector<bool> > buildResidueMask(std::vector<unsigned int> _residueState, std::vector< std::vector<unsigned int> > _residueRotamers) {
	...
	for (unsigned int i = 0; i < _residueState.size(); i++) {
		...
		//check to make sure that there are some true rotamers
		uint counter = 0;
